...
public class IPFS {
    ...
    public IPFS(String host, int port, Map<String, String> headers, boolean ssl, String version) {
        this(host, port, version, ssl, headers);
    }

    public IPFS(String host, int port) {
        this(host, port, "/api/v0/", false, new HashMap<>());
    }
    ...
    public IPFS(MultiAddress addr) {
        this(addr.getHost(), addr.getTCPPort(), "/api/v0/", detectSSL(addr), new HashMap<>());
    }

    public IPFS(String host, int port, String version, boolean ssl, Map<String, String> headers) {
        this.host = host;
        this.port = port;
    ...
    private byte[] retrieve(String path) throws IOException {
        ...
        return IPFS.get(target, headers);
    }

    private static byte[] get(URL target, Map<String, String> headers) throws IOException {
    ...
    private InputStream retrieveStream(String path) throws IOException {
        ...
        return IPFS.getStream(target, headers);
    }

    private static InputStream getStream(URL target, Map<String, String> headers) throws IOException {
        HttpURLConnection conn = (HttpURLConnection) target.openConnection();
        conn.setRequestMethod("GET");
        conn.setRequestProperty("Content-Type", "application/json");
        // add headers
